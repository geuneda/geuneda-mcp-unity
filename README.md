...
**UI (TextMeshPro)**
- `create_ui_element`: UI 요소 생성 (Canvas, Button, Text, Image, Panel, Slider, Toggle, InputField, Dropdown, ScrollView) - Canvas 및 EventSystem 자동 설정, Text/Button/Toggle/InputField/Dropdown은 TextMeshPro(TMP) 컴포넌트 사용
- `modify_ui_element`: UI 요소 속성 수정 (텍스트, 폰트 크기, 색상, 위치, 크기, 활성 상태) - TextMeshPro 텍스트 컴포넌트 지원
...
**프로파일러**
- `get_memory_snapshot`: 메모리 사용량 스냅샷 (총 할당/예약/미사용, Mono 힙, 그래픽스 드라이버, 에셋별 상세 분석 옵션)
- `get_rendering_stats`: 렌더링 통계 조회 (드로우콜, 배치, 셋패스콜, 삼각형, 버텍스, 섀도우 캐스터)
- `get_frame_timing`: CPU/GPU 프레임 타이밍 조회 (현재 FPS, 타겟 FPS, vSync, 프레임별 CPU/GPU 시간)
- `get_profiler_data`: 프로파일러 프레임 데이터 조회 (프레임별 상위 CPU 샘플, 호출 수, GC 할당, 카테고리 필터링)
- `get_profiler_report`: 종합 프로파일러 리포트 (메모리 + 렌더링 + 프레임 타이밍 + CPU 샘플을 단일 응답으로 통합)
